...
class HomeController extends GetxController {
  ...
  try {
    ...
    followingIds.add(currentUserId); // إضافة المستخدم الحالي نفسه

    // استخدام استعلام "or" لجلب المنشورات بناءً على user_id
    final String orCondition =
        followingIds.map((id) => 'user_id.eq.$id').join(',');

    final List<dynamic> response = await SupabaseService.SupabaseClient
        .from("posts")
        .select('''
        ...
        print("The post are ${jsonEncode(response)}");
    ...
    SupabaseService.SupabaseClient.channel('public:posts')
        .onPostgresChanges(
          ...
          callback: (payload) async {
            ...
            // التحقق من أن المنشور من المستخدم الحالي أو من المتابعين
