...
class ProfileController extends GetxController {
  ...
  void fetchUserPosts(String userId) async {
    ...
    id, content, image, created_at, comment_count, like_count, user_id, user:user_id (email, metadata), likes:likes (user_id, post_id)
...
''').eq("user_id", userId).order("id", ascending: false);
  ...
  try {
    ...
    if (updateResponse == null || updateResponse.isEmpty) {
      print("Error: Update failed, no rows were returned.");
    } else {
      print("User followed successfully.");

      // تحديث حالة المستخدم الحالية لعرضها في الواجهة بشكل فوري
      user.update((u) {
